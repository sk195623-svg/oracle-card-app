...
def create_card_design(image_path: str, card_name: str, output_path: Optional[str] = None) -> str:
    ...
    draw.rounded_rectangle([(8, 8), (card_width - 8, card_height - 8)], radius=28, outline=(180, 160, 120), width=6)
    draw.rounded_rectangle([(22, 22), (card_width - 22, card_height - 22)], radius=24, outline=(220, 205, 170), width=2)
...
def generate_ai_card(theme: str) -> Dict:
    ...
    response = client.responses.create(model="gpt-5", input=build_card_generation_prompt(theme))
...
def optimize_image_file(input_path: str, max_width: int = 512, max_height: int = 768, png_compress_level: int = 9) -> str:
    ...
    img.save(input_path, format="PNG", optimize=True, compress_level=png_compress_level)
    return input_path
...
def attach_image_to_saved_card(target_card: Dict, image_path: str, card_image_path: str = "") -> bool:
    cards = load_cards()
...
def display_one_card_with_effect(card: dict) -> None:
    ...
    with area.container():
        display_card(card, show_actions=False, index=0, theme=st.session_state.get("ai_card_theme", "やさしい癒し"))


def display_three_cards_sequential(cards_to_show: List[Dict]) -> None:
    labels = ["過去", "現在", "未来"]
    for i, card in enumerate(cards_to_show[:3]):
        st.markdown(f"### {labels[i]}")
        display_card(card, show_actions=False, index=i, theme=st.session_state.get("ai_card_theme", "やさしい癒し"))


def display_card(card: Dict, show_actions: bool = False, index: int = 0, theme: str = "やさしい癒し") -> None:
    st.markdown('<div class="result-card">', unsafe_allow_html=True)
    st.markdown(f"<div class='card-title'>{card.get('name', '名称未設定')}</div>", unsafe_allow_html=True)

    image_path = get_display_image_path(card)
    if image_path:
    ...
    st.markdown(f"<div class='card-message'>{card.get('message', '')}</div>", unsafe_allow_html=True)

    if show_actions:
        col1, col2 = st.columns(2)

        with col1:
            if st.button("このカード画像を生成", key=f"gen_img_{index}", use_container_width=True):
                try:
                    ...
                    attach_image_to_saved_card(card, image_path=path, card_image_path=card_path_rel)
        ...
        with col2:
            if st.button("このカードを削除", key=f"delete_card_{index}", use_container_width=True):
                cards_now = load_cards()
                idx = find_card_index(cards_now, card)
                if idx >= 0:
                    deleted_name = cards_now[idx].get("name", "名称未設定")
                    cards_now.pop(idx)
                    save_cards(cards_now)
...
with st.expander("🔒 管理画面を開く", expanded=False):
    admin_pass = st.text_input("管理用パスワード", type="password", key="admin_pass_input")

    if st.button("管理画面を開く", use_container_width=True, key="unlock_admin_button"):
        if admin_pass == ADMIN_PASSWORD:
            st.session_state["admin_unlocked"] = True
            st.success("管理画面を開きました")
        else:
            st.session_state["admin_unlocked"] = False
            st.error("パスワードが違います")
...
if tab2 is not None:
    with tab2:
            ...
            if st.button("AIで1枚＋画像も生成", key="one_ai_generate_with_image", use_container_width=True):
                try:
                    with st.spinner("AIカード生成中..."):
                        card = add_ai_card_to_json(theme=st.session_state.get("ai_card_theme", "やさしい癒し"))

                    with st.spinner("画像生成中..."):
                        image_path = generate_card_image(card, theme=st.session_state.get("ai_card_theme", "やさしい癒し"))
                    ...
                    with st.spinner("カードデザイン作成中..."):
                        card_image_abs = create_card_design(image_path=abs_image_path, card_name=card.get("name", "無題"))

                    card_image_rel = str(Path("images") / Path(card_image_abs).name)

                    attach_image_to_saved_card(card, image_path=image_path, card_image_path=card_image_rel)
        ...
        bulk_count = st.number_input("生成枚数", min_value=1, max_value=20, value=5, step=1, key="bulk_ai_card_count_v2")

        generate_images = st.checkbox("カード画像もAI生成する", value=True, key="generate_ai_images")
        ...
        cards_without_image = [card for card in latest_cards_for_images if not get_display_image_path(card)]
                ...
                if not cards_without_image:
                    ...
                    try:
                        ...
                        for i, card in enumerate(target_cards, start=1):
                            path = generate_card_image(card, theme=st.session_state.get("ai_card_theme", "やさしい癒し"))
        ...
        if not latest_cards:
            ...
            for idx, card in enumerate(filtered_cards):
                ...
                display_card(card, show_actions=True, index=idx, theme=st.session_state.get("ai_card_theme", "やさしい癒し"))
